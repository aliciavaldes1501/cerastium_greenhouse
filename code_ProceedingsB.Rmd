---
title: Small-scale genetic differentiation along a geothermal heating gradient suggests local adaptation in mean flowering time but not in plasticity
...
There is additive genetic variation in vegetative growth and flowering time at a given temperature.

### Models

```{r}
model1_veg_uh<-lmer(mean_sh_growth~1+(1|sire)+(1|dam),
                       #(1|sire:dam) # Removed to avoid singular fit
                     subset(data_exp,treat=="unheated"))
model1_veg_h<-lmer(mean_sh_growth~1+(1|sire)+(1|dam)+(1|sire:dam),
...
model1_MFD_h<-lmer(MFD~1+(1|sire)+(1|dam)+(1|sire:dam),
                 subset(data_exp,treat=="heatmat"))
summary(model1_veg_uh)
summary(model1_veg_h)
...
```{r}
Variance_veg_uh<-as.data.frame(VarCorr(model1_veg_uh))[,c(1,4)]
...
Variance_veg_h<-as.data.frame(VarCorr(model1_veg_h))[,c(1,4)]
...
### Appendix S5: LRTs for variance components

```{r}
ranova(model1_veg_uh)
ranova(model1_veg_h)
...
Plants respond plastically to increased temperatures by starting vegetative growth and flowering earlier, and there is additive genetic variation in plasticity of these traits.

### Models

```{r}
model2_veg<-lmer(mean_sh_growth~treat+
                    (1|sire)+(1|dam)+
...
Variance_veg_treat <- as.data.frame(VarCorr(model2_veg))[,c(1,4)]  
...
### Appendix S6: LRTs for variance components
...
```{r}
data_fig2<-rbind(
  data.frame(ggpredict(model2_veg,terms="treat"))%>%mutate(variable="veg"),
...
```{r}
AppS7<-ggplot(data=Props_var_temp_mf%>%
             filter(term!="temp_sire+temp_dam"&term!="Full")%>%
             mutate(term=factor(term,levels=c("temp_sire","temp_dam"),
                                labels=c("Sire origin temperature",
                                           "Dam origin temperature")),
  ...
  theme(legend.position="top",legend.title=element_blank())
AppS7
ggsave(file="output/figures/AppS7.tiff", plot=AppS7,width=7,height=8,dpi=300,
       compression="lzw")
```
...
tab_model(model3_veg_uh_midP,model3_FFD_uh_midP,model3_MFD_uh_midP,
          ...
          show.r2=T,show.icc=F,show.re.var=F,show.ngroups=F,digits=3,
          dv.labels=c("Vegetative growth","FFD","MFD","LFD"),
          file="output/tables/AppS2_uh.html")
tab_model(model3_veg_h_midP,model3_FFD_h_midP,model3_MFD_h_midP,
          model3_LFD_h_midP,
          transform=NULL,show.intercept=F,show.ci=F,show.se=T,show.stat=T,
          show.r2=T,show.icc=F,show.re.var=F,show.ngroups=F,digits=3,
          dv.labels=c("Vegetative growth","FFD","MFD","LFD"),
          file="output/tables/AppS2_h.html")
```

### Models breeding values (Appendix S4)

Get breeding values/BLUPs from models in Prediction 1.

```{r}
averages_resp<-rbind(
  data.frame(average=as.numeric(fixef(model1_veg_uh)))%>%
    mutate(name="veg_uh_resp"),
  data.frame(average=as.numeric(fixef(model1_veg_h)))%>%
    mutate(name="veg_h_resp"),
...
breedV_resp_sires<-left_join(
    ...
    left_join(
      left_join(
        left_join(
          left_join(
            left_join(
              ranef(model1_veg_uh)$sire%>%rownames_to_column(var="sire")%>%
                rename(breedV_veg_uh_resp_sire="(Intercept)"),
              ranef(model1_veg_h)$sire%>%rownames_to_column(var="sire")%>%
                rename(breedV_veg_h_resp_sire="(Intercept)")),
  ...
  pivot_longer(cols=breedV_veg_uh_resp_sire:breedV_LFD_h_resp_sire,
...
breedV_resp_dams<-left_join(
    ...
    left_join(
      left_join(
        left_join(
          left_join(
            left_join(
              ranef(model1_veg_uh)$dam%>%rownames_to_column(var="dam")%>%
                rename(breedV_veg_uh_resp_dam="(Intercept)"),
              ranef(model1_veg_h)$dam%>%rownames_to_column(var="dam")%>%
                rename(breedV_veg_h_resp_dam="(Intercept)")),
  ...
  pivot_longer(cols=breedV_veg_uh_resp_dam:breedV_LFD_h_resp_dam,
               names_to=c("remove","var","treat","type","comp"),names_sep="_",
               values_to="breedV")%>%
  dplyr::select(-remove)%>%
  left_join(averages_resp)%>%mutate(breedV_av=breedV+average)
...
```{r}
mod_bV_resp_temp_sire_veg_uh<-lm(breedV~temp_sire,
                                 subset(breedV_resp_sires,
                                        var=="veg"&treat=="uh"))
mod_bV_resp_temp_sire_veg_h<-lm(breedV~temp_sire,
                                 subset(breedV_resp_sires,
                                        var=="veg"&treat=="h"))
...
```{r}
summary(mod_bV_resp_temp_sire_veg_uh)
summary(mod_bV_resp_temp_sire_veg_h)
...
```{r}
figS4A<-plot(ggpredict(mod_bV_resp_temp_sire_MFD_h,terms="temp_sire"),
             add.data=T)+
...
```{r}
mod_bV_resp_temp_dam_veg_uh<-lm(breedV~temp_dam,
                                 subset(breedV_resp_dams,
                                        var=="veg"&treat=="uh"))
mod_bV_resp_temp_dam_veg_h<-lm(breedV~temp_dam,
                                 subset(breedV_resp_dams,
                                        var=="veg"&treat=="h"))
...
```{r}
summary(mod_bV_resp_temp_dam_veg_uh)
summary(mod_bV_resp_temp_dam_veg_h)
...
averages_plast<-rbind(
  data.frame(ggpredict(model2_veg))[1:2]%>%
    pivot_wider(names_from=treat.x,values_from=treat.predicted)%>%
    mutate(slope=heatmat-unheated,name="veg_plast"), 
...
```{r}
breedV_plast_dams<-left_join(
  ranef(model2_veg)$`treat:dam`%>%
    rownames_to_column(var="treat_dam")%>%
    rename(breedV_veg_plast_dam="(Intercept)")%>%
    separate(treat_dam,into=c("treat","dam"))%>%
    pivot_wider(names_from="treat",
                values_from="breedV_veg_plast_dam",
                names_prefix="breedV_veg_plast_dam_"),
  ranef(model2_LFD)$`treat:dam`%>%
    ...
    pivot_longer(cols=breedV_veg_plast_dam_heatmat:breedV_LFD_plast_dam_unheated,
               names_to=c("remove","var","type","comp","treat"),names_sep="_",
               values_to="breedV")%>%
  dplyr::select(-remove)%>%
  pivot_wider(names_from="treat",values_from="breedV",names_prefix="breedV_")%>%
...
```{r}
mod_bV_plast_temp_dam_veg<-lm(breedV~temp_dam,
                               subset(breedV_plast_dams,var=="veg"))
mod_bV_plast_temp_dam_LFD<-lm(breedV~temp_dam,
                                 subset(breedV_plast_dams,var=="LFD"))
```

```{r}
summary(mod_bV_plast_temp_dam_veg)
...
Combining our results regarding heritability of flowering time with previous field estimates of temperature-dependent phenotypic selection on first flowering date, we predict evolutionary change at different soil temperatures.

Selection gradients for different temperatures and sd for FFD in 2018 from reference [22]:

```{r}
selgrads_temp<-read_csv("data/clean/selgrads_temp_from_22.csv")
sd_FFD_2018<-as.tibble(13.65736)
```
...
R_UH<-her_FFD_uh* # Heritability for UH treatment
  ...
  sd_FFD_2018$value # sd for FFD in 2018
...
Calculate R at 8ºC (more similar to unheated conditions in the field):
