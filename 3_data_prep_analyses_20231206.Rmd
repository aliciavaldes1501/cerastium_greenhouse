...
fig1<-grid.arrange(legend_fig1,fig1_without_legend,ncol=1,
                   heights=c(0.1,0.9))
plot(fig1)
ggsave(file="output/figures/fig1.tiff", plot=fig1,width=10,height=7,dpi=300,
       compression="lzw")
```
...
fig3<-ggplot()+
  geom_bar(data=Props_var_temp_mf%>%
           ...
           stat="identity",color="black",size=0.2)+facet_wrap(~treat)+
  ...
  theme(legend.position="top",legend.title=element_blank())
fig3
ggsave(file="output/figures/fig3.tiff", plot=fig3,width=7,height=6,dpi=300,
       compression="lzw")
```
...
```{r}
figS1A<-plot(ggpredict(mod_bV_elev_temp_sire_MFD_h,terms="temp_sire"),add.data=T)+
  xlab("Sire origin temperature (ºC)")+
  ylab("Sire breeding value\nfor elevation")+
  my_theme()+ggtitle(NULL)
figS1A
ggsave(file="output/figures/figS1A.tiff", plot=figS1A,width=5,height=4,dpi=300,
       compression="lzw")
```
...
breedV_slope_sires<-left_join(
  left_join(
    left_join(
      ranef(model1b_veg)$`treat:sire`%>%
        rownames_to_column(var="treat_sire")%>%
        rename(breedV_veg_slope_sire="(Intercept)")%>%
        separate(treat_sire,into=c("treat","sire"))%>%
        pivot_wider(names_from="treat",
                    values_from="breedV_veg_slope_sire",
                    names_prefix="breedV_veg_slope_sire_"),
  ...
  ranef(model1b_LFD)$`treat:sire`%>%
    rownames_to_column(var="treat_sire")%>%
    rename(breedV_LFD_slope_sire="(Intercept)")%>%
    separate(treat_sire,into=c("treat","sire"))%>%
    pivot_wider(names_from="treat",
                values_from="breedV_LFD_slope_sire",
                names_prefix="breedV_LFD_slope_sire_")
  )%>%
  pivot_longer(cols=breedV_veg_slope_sire_heatmat:breedV_LFD_slope_sire_unheated,
...
breedV_slope_dams<-left_join(
  left_join(
    left_join(
      ...
      ranef(model1b_FFD)$`treat:dam`%>%
        rownames_to_column(var="treat_dam")%>%
        rename(breedV_FFD_slope_dam="(Intercept)")%>%
        separate(treat_dam,into=c("treat","dam"))%>%
        pivot_wider(names_from="treat",
                    values_from="breedV_FFD_slope_dam",
                    names_prefix="breedV_FFD_slope_dam_")
      ),
    ranef(model1b_MFD)$`treat:dam`%>%
      rownames_to_column(var="treat_dam")%>%
      rename(breedV_MFD_slope_dam="(Intercept)")%>%
      separate(treat_dam,into=c("treat","dam"))%>%
      pivot_wider(names_from="treat",
                  values_from="breedV_MFD_slope_dam",
                  names_prefix="breedV_MFD_slope_dam_")
    ),
...
```{r}
mod_bV_slope_temp_sire_veg<-lm(breedV~temp_sire,
                               subset(breedV_slope_sires,var=="veg"))
...
mod_bV_slope_temp_sire_LFD<-lm(breedV~temp_sire,
                                 subset(breedV_slope_sires,var=="LFD"))
```

```{r}
summary(mod_bV_slope_temp_sire_veg)
summary(mod_bV_slope_temp_sire_FFD)
summary(mod_bV_slope_temp_sire_MFD)
summary(mod_bV_slope_temp_sire_LFD)
# All NS
```
...
mod_bV_slope_temp_dam_FFD<-lm(breedV~temp_dam,
                                 subset(breedV_slope_dams,var=="FFD"))
mod_bV_slope_temp_dam_MFD<-lm(breedV~temp_dam,
                                 subset(breedV_slope_dams,var=="MFD"))
...
summary(mod_bV_slope_temp_dam_FFD)
summary(mod_bV_slope_temp_dam_MFD)
